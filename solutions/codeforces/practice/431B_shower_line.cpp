// CodeForces Problem Link: https://codeforces.com/contest/431/problem/B
...
const int32 MAX32 = 2147483647, MIN32 = -2147483647;
// const int64 MAX64 = 9223372036854775807, MIN64 = -9223372036854775807;
...
void solve() {
  ...
  EACH(i, g) EACH(j, i) cin >> j;
  int ans = -1, res, order[5];
  FOR(i, 5) order[i] = i;
  do {
    res = g[order[0]][order[1]] + g[order[1]][order[0]];
    res += g[order[2]][order[3]] + g[order[3]][order[2]];
    res += g[order[1]][order[2]] + g[order[2]][order[1]];
    res += g[order[3]][order[4]] + g[order[4]][order[3]];
    res += g[order[2]][order[3]] + g[order[3]][order[2]];
    res += g[order[3]][order[4]] + g[order[4]][order[3]];
    ans = max(ans, res);
