// CodeForces Problem Link: https://codeforces.com/problemset/problem/165/A
...
void solve() {
    ...
    FOR(i, n) {
        ...
        FOR(j, n) {
            ...
            if (x[j] == x[i] && y[j] < y[i]) bottom = true;
            if (x[j] == x[i] && y[j] > y[i]) top = true;
        }
        ans += (right && left && bottom && top);
    }
    cout << ans << nl;
}
