// CodeForces Problem Link: https://codeforces.com/contest/144/problem/A
...
void solve() {
    int n; cin >> n;
    pair<int, int> mx = {SMALL, -1}, mn = {BIG, -1};
    FOR(i, n) {
        int num; cin >> num;
        if (num > mx.first) {
            mx.first = num;
    ...
    int ans = mx.second + (n - mn.second - 1);
    if (mx.second > mn.second) ans--;
    cout << ans << nl;
}
