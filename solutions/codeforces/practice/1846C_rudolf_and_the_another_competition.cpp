// CodeForces Problem Link: https://codeforces.com/problemset/problem/1846/C
...
void solve() {
  ll n, m, h; cin >> n >> m >> h;
  vector<array<ll, 2>> scores(n);
  array<ll, 2> r;
  FOR(i, n) {
    vector<ll> times(m);
    EACH(j, times) cin >> j;
    sort(all(times));
    ll solves = 0, penalty = 0, time = 0;
    FOR(j, m) {
      time += times[j];
      if (time <= h) {
        penalty += time;
    ...
    array<ll, 2> arr = {solves, penalty};
    if (i == 0) r = arr;
    scores[i] = arr;
  }
  sort(all(scores), [](array<ll, 2> &arr1, array<ll, 2> &arr2) {
    if (arr1[0] == arr2[0]) {
