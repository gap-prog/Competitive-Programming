// CodeForces Problem Link: https://codeforces.com/contest/1749/problem/B
...
void solve() {
  ...
  FOR(i, n) cin >> monsters[i].second;
  FOR(i, n) cin >> monsters[i].first;
  sort(all(monsters));
  ll ans = monsters[0].second;
  if (n > 1) {
    FOR(i, 1, n) {
      monsters[i].second += monsters[i - 1].first;
