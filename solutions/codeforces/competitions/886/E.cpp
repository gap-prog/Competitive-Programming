// CodeForces Problem Link: https://codeforces.com/contest/1850/problem/E
...
void solve() {
  ll n, c; cin >> n >> c;
  __int128 sum = 0, square = 0;
  FOR(i, n) {
    ll s; cin >> s;
    sum += s;
    square += (s * s);
  }
  __int128 A = 4 * n, B = 4 * sum, C = square - c;
  __int128 w = (-B + sqrtl(((ll) B * B) - (4 * A * C))) / (2 * A);
  cout << (ll) w << nl;
}
