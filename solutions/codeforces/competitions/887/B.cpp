// CodeForces Problem Link: https://codeforces.com/contest/1853/problem/B
...
void solve() {
  ...
  ll ans = 0;
  FOR(i, n + 1) {
    ll res = n - p.first * ((ll) i);
    if (res % p.second == 0) if (res / p.second >= i) ans++;
  }
  cout << ans << nl;
}
...
int main() {
  setIO();
  pair<ll, ll> p1 = {1, 0}, p2 = {0, 1};
  v[1] = p1;
  v[2] = p2;
  FOR(i, 3, MAX - 100 + 1) {
    p1 = v[i - 2]; p2 = v[i - 1];
    pair<ll, ll> p3;
    p3.first = p2.first + p1.first; p3.second = p2.second + p1.second;
    v[i] = p3;
    p1 = p2;
    p2 = p3;
  }
